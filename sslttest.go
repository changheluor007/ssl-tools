...
func certSetup(SerialNumber int64, commonName *string, years int, state bool, template0 *x509.Certificate, privkey0 *rsa.PrivateKey) (*rsa.PrivateKey, *x509.Certificate, *x509.Certificate, *bytes.Buffer, *bytes.Buffer) {
	...
	//plan one
	//cert, err := x509.CreateCertificate(rand.Reader, template, template0, &privkey.PublicKey, privkey0)
	//checkErr(err)
	...
	//})
	//cert1, err := x509.ParseCertificate(cert)
	//checkErr(err)

	//plan two
	cert, certPEM, privKeyPEM := genCert(template, template0, privkey, privkey0)
	return privkey, template, cert, certPEM, privKeyPEM
}

func genCert(template, parent *x509.Certificate, privateKey *rsa.PrivateKey, privateKey0 *rsa.PrivateKey) (*x509.Certificate, *bytes.Buffer, *bytes.Buffer) {
	caBytes, err := x509.CreateCertificate(rand.Reader, template, parent, &privateKey.PublicKey, privateKey0)
	if err != nil {
